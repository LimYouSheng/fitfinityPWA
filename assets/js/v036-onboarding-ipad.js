...
(function(){
  ...
  const couplePeopleSeedV36={
    'Mei & Aaron':[
      {name:'Mei',phone:'+65 9134 5678',email:'mei@example.com',age:'34',gender:'Female',emergencyName:'Aaron Lee',emergencyPhone:'+65 9011 2233',health:'Mild shoulder tightness after desk work. Keep pressing pain-free and include upper-back mobility.'},
      {name:'Aaron',phone:'+65 9245 6789',email:'aaron@example.com',age:'35',gender:'Male',emergencyName:'Mei Lee',emergencyPhone:'+65 9134 5678',health:'No current injuries or movement limitations reported.'}
    ],
    'Siti & Hakim':[
      {name:'Siti',phone:'+65 9189 0123',email:'siti@example.com',age:'33',gender:'Female',emergencyName:'Hakim Rahman',emergencyPhone:'+65 9066 7788',health:'No current injuries or movement limitations reported.'},
      {name:'Hakim',phone:'+65 9290 1234',email:'hakim@example.com',age:'35',gender:'Male',emergencyName:'Siti Rahman',emergencyPhone:'+65 9189 0123',health:'Monitor knee comfort during deeper squat ranges; reduce depth if symptoms appear.'}
    ],
    'Jason & Claire':[
      {name:'Jason',phone:'+65 9234 5678',email:'jason@example.com',age:'37',gender:'Male',emergencyName:'Claire Goh',emergencyPhone:'+65 9111 2233',health:'No current injuries or movement limitations reported.'},
      {name:'Claire',phone:'+65 9345 6789',email:'claire@example.com',age:'35',gender:'Female',emergencyName:'Jason Goh',emergencyPhone:'+65 9234 5678',health:'Avoid aggressive overhead range when the shoulder feels tight; use a comfortable pressing path.'}
    ]
  };
  Object.entries(couplePeopleSeedV36).forEach(([name,people])=>{
    ...
    d.phone=people[0].phone;d.email=people[0].email;d.age=people[0].age;d.gender=people[0].gender;
  ...
  function peopleForClientV36(name){
    ...
    return [{name:first||name,phone:d.phone||'—',email:d.email||'—',age:d.age||'—',gender:d.gender||'—',emergencyName:d.emergencyName||'—',emergencyPhone:d.emergencyPhone||'—',health:d.health||'No health / limitation notes recorded.'}];
  }
  window.peopleForClientV36=peopleForClientV36;

  function personFieldsV36(i){
    if(i===1)return {name:'newClientName',phoneCountry:'newClientPhoneCountry',phone:'newClientPhone',email:'newClientEmail',age:'newClientAge',gender:'newClientGender',emergencyName:'newClientEmergencyName',emergencyCountry:'newClientEmergencyPhoneCountry',emergencyPhone:'newClientEmergencyPhone',health:'newClientHealthV36'};
    return {name:'newClient2NameV36',phoneCountry:'newClient2PhoneCountryV36',phone:'newClient2PhoneV36',email:'newClient2EmailV36',age:'newClient2AgeV36',gender:'newClient2GenderV36',emergencyName:'newClient2EmergencyNameV36',emergencyCountry:'newClient2EmergencyPhoneCountryV36',emergencyPhone:'newClient2EmergencyPhoneV36',health:'newClient2HealthV36'};
  }
  function readOnboardPersonV36(i){
    const f=personFieldsV36(i);
    return {name:val(f.name),phone:composePhoneV36(f.phoneCountry,f.phone),email:val(f.email),age:val(f.age),gender:el(f.gender)?.value||'',emergencyName:val(f.emergencyName),emergencyPhone:composePhoneV36(f.emergencyCountry,f.emergencyPhone),health:val(f.health)};
  }
  function personCompleteV36(i){const p=readOnboardPersonV36(i);return !!(p.name&&p.phone&&p.email&&p.age&&p.gender&&p.emergencyName&&p.emergencyPhone&&p.health)}
  ...
  function readCoupleEditPersonV36(i){const n=i+1;return {name:val(`coupleEditName${n}V36`),phone:composePhoneV36(`coupleEditPhoneCountry${n}V36`,`coupleEditPhone${n}V36`),email:val(`coupleEditEmail${n}V36`),age:val(`coupleEditAge${n}V36`),gender:el(`coupleEditGender${n}V36`)?.value||'',emergencyName:val(`coupleEditEmergencyName${n}V36`),emergencyPhone:composePhoneV36(`coupleEditEmergencyCountry${n}V36`,`coupleEditEmergencyPhone${n}V36`),health:peopleForClientV36(el('ownerClientName')?.textContent.trim()||'')[i]?.health||''}}
  function completePersonObjV36(p){return !!(p?.name&&p.phone&&p.email&&p.age&&p.gender&&p.emergencyName&&p.emergencyPhone)}
  ...
  window.toggleClientMainEdit=function(){
    ...
    const d=extendedClientData(name);d.people=people;Object.assign(d,{phone:people[0].phone,email:people[0].email,age:people[0].age,gender:people[0].gender,emergencyName:people[0].emergencyName,emergencyPhone:people[0].emergencyPhone,partnerPhone:people[1].phone});
